...
export async function upsertInvoiceSeriesAction(input: {
  ...
  const session = await ensureAdmin();
  if (!input.code.trim()) throw new Error("Código de serie obligatorio");
...
export async function createInvoiceFromContractV2Action(
  ...
  if (!series) {
    throw new Error(
      "No hay serie de facturación. Crea una en /configuracion/facturacion",
    );
  }
 ...
 */
export async function issueInvoiceV2Action(invoiceId: string): Promise<void> {
  const session = await ensureAdmin();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const admin = createAdminClient() as any;
  ...
  if (inv.total_cents <= 0) {
    throw new Error("La factura no tiene importe");
  }
  if (inv.company_id !== session.company_id) {
    throw new Error("Factura de otra empresa");
  }
  ...
  if (!issuerNif || !issuerName) {
    throw new Error(
      "Faltan datos fiscales de la empresa (CIF y razón social). Configúralos en /configuracion/fiscal",
    );
  }
...
export async function cancelInvoiceV2Action(
  ...
  const session = await ensureAdmin();
  if (!reason.trim()) throw new Error("Motivo de anulación obligatorio");
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const admin = createAdminClient() as any;
