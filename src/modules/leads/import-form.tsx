...
function parseCsv(text: string): ImportRow[] {
  // Parser CSV simple: separador , y comillas dobles para campos con comas
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const row: ImportRow = { party_kind: "individual" };
    headers.forEach((h, j) => {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
          <p className="text-xs text-muted-foreground">
            Cabeceras admitidas: <code>tipo, razon_social, nombre, apellidos, email, telefono, dni, origen, notas</code>.
            Tipo: <code>individual</code> o <code>company</code>. Los duplicados (DNI/email/teléfono) se ignoran automáticamente.
          </p>
          <input
            type="file"
            accept=".csv,text/csv"
