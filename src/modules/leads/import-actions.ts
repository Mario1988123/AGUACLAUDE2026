...
/**
 * Importa leads desde un array de filas. Si phone/email/tax_id duplican un
 * lead/cliente existente, se cuenta como duplicado y NO se inserta.
 */
export async function importLeadsAction(rows: ImportRow[]): Promise<ImportResult> {
  ...
  // Recopilar todos los identificadores únicos para una sola consulta de dedupe
  const phones = new Set<string>();
  const emails = new Set<string>();
  const taxIds = new Set<string>();
  for (const r of rows) {
    if (r.phone_primary) {
      const norm = normalizeSpanishPhone(r.phone_primary) ?? r.phone_primary.trim();
      phones.add(norm);
    }
  ...
  // Cargar duplicados existentes en una pasada
  ...
  // Insertar uno a uno (BD valida y permite continuar tras error individual)
  for (let idx = 0; idx < rows.length; idx++) {
    ...
    try {
      ...
      const origin = (VALID_ORIGINS as readonly string[]).includes(r.origin ?? "")
        ? r.origin
        : "other";
      ...
      // Marcar para no duplicar en el siguiente lote dentro del mismo CSV
