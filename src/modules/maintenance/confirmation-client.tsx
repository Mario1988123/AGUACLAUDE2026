...
export function ConfirmationClient({
  ...
  // Para el reschedule, propuesta inicial: 7 días después de la fecha
  // actual del job (ofrecemos un slot por defecto razonable).
  const proposedNext = new Date(
    new Date(scheduledAt).getTime() + 7 * 86400_000,
  );
  ...
  function submitReschedule() {
    ...
    startTransition(async () => {
      const r = await customerRescheduleAction(token, iso);
      if (!r.ok) {
        setError(r.message);
        return;
      }
    ...
    <div className="space-y-3">
      ...
      {view === "reschedule" && (
        <div className="space-y-3 rounded-xl border-2 border-sky-200 bg-sky-50/30 p-4">
          ...
          <p className="text-[11px] text-muted-foreground">
            Tu propuesta se enviará a nuestro equipo y te confirmaremos disponibilidad.
          </p>
      ...
      {view === "postpone" && (
        <div className="space-y-3 rounded-xl border-2 border-amber-200 bg-amber-50/40 p-4">
          ...
          />
          <p className="text-[11px] text-muted-foreground">
            Nos pondremos en contacto contigo lo antes posible para coordinar
            una nueva fecha.
          </p>
          <div className="flex gap-2">
            <button
