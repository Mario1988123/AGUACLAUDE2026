...
type Section = "data" | "address" | "iban" | "photo" | null;
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b p-4">
          <div>
            <h2 className="text-lg font-bold">Antes de firmar</h2>
            <p className="text-xs text-muted-foreground">
              Revisa los datos del cliente. Lo crítico debe estar completo.
            </p>
          </div>
          <button
            type="button"
        ...
        {loading || !readiness ? (
          ...
          <div className="space-y-3 p-4">
            ...
            <CheckRow
              ...
              critical
              onEdit={() => setEditing(editing === "data" ? null : "data")}
              expanded={editing === "data"}
            >
              {editing === "data" && (
                <CustomerEditForm
            ...
            <CheckRow
              ...
              status={
                readiness.primary_address
                  ? `${readiness.primary_address.street ?? ""}${readiness.primary_address.street_number ? ` ${readiness.primary_address.street_number}` : ""}, ${readiness.primary_address.city ?? ""}`.trim()
                  : "Falta dirección"
              }
              critical
            ...
            <CheckRow
              ...
              warning={readiness.checks.has_iban && !readiness.checks.iban_validated}
              status={
                readiness.primary_bank
                  ? readiness.primary_bank.is_validated
                    ? `${readiness.primary_bank.iban.slice(0, 8)} ****`
                    : "ES00 · pendiente (firmable)"
                  : "Falta IBAN"
              }
              critical
              onEdit={() => setEditing(editing === "iban" ? null : "iban")}
              expanded={editing === "iban"}
            >
              {editing === "iban" && (
                <IbanAddForm
                  customerId={readiness.customer.id}
            ...
            <CheckRow
              ...
              status={
                readiness.checks.has_id_photo
                  ? "Foto subida"
                  : "Recomendado: foto del DNI/NIE"
              }
              critical={false}
              onEdit={() => setEditing(editing === "photo" ? null : "photo")}
              expanded={editing === "photo"}
            >
              {editing === "photo" && (
                <PhotoUploadInline
            ...
            {readiness.blockers.length > 0 && (
              <div className="rounded-xl border-2 border-destructive bg-destructive/5 p-3">
                <p className="flex items-center gap-1.5 text-sm font-bold text-destructive">
                  <AlertTriangle className="h-4 w-4" /> No puedes firmar todavía
                </p>
            ...
            {readiness.warnings.length > 0 && (
              <div className="rounded-xl border-2 border-amber-300 bg-amber-50 p-3">
                <p className="flex items-center gap-1.5 text-sm font-bold text-amber-900">
                  <AlertTriangle className="h-4 w-4" /> Recomendado completar
                </p>
                ...
                <p className="mt-1 text-xs text-amber-800">
                  Puedes firmar igualmente, pero conviene tenerlo antes.
                </p>
              </div>
            )}
          </div>
