import { CalendarCheck2, Phone, ShieldCheck, Wrench } from "lucide-react";
import { getPublicInstallationView } from "@/modules/installations/public-confirmation-actions";
import { InstallationConfirmationClient } from "@/modules/installations/installation-confirmation-client";

export const dynamic = "force-dynamic";

export default async function PublicInstallationConfirmationPage({
  params,
  searchParams,
}: {
  ...
  const view = await getPublicInstallationView(token);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-sky-50 to-white p-6">
        <div className="w-full max-w-md rounded-2xl border border-rose-200 bg-white p-8 text-center shadow-lg">
          <h1 className="text-xl font-bold text-rose-700">Enlace no válido</h1>
      ...
      <div className="mx-auto max-w-xl">
        <div className="rounded-2xl border border-border bg-white p-6 shadow-lg sm:p-8">
          ...
          <p className="mt-2 flex items-center gap-1.5 text-sm text-muted-foreground">
            ...
            {view.token.used
              ? "Tu instalación ya está registrada. Si necesitas un cambio, llámanos."
              : "Estamos preparando la instalación de tu equipo."}
          </p>
          ...
          <div className="my-6 rounded-2xl border-2 border-sky-200 bg-sky-50 p-5">
            ...
            {job.customer_address && (
              <p className="mt-1 text-xs text-sky-800">📍 {job.customer_address}</p>
            )}
          </div>
          ...
          {view.token.used ? (
            ...
            <InstallationConfirmationClient
              token={token}
