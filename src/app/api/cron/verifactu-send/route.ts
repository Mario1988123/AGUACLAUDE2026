...
import { sendToAeat } from "@/modules/invoices/verifactu-aeat";
import {
  buildRegistroFacturacionAltaXml,
  buildSoapEnvelope,
} from "@/modules/invoices/verifactu-xml";
...
/**
 * Cron de envío Verifactu a AEAT.
 *
 * Ejecutar cada 5-15 minutos. Procesa hasta 50 envíos pendientes por
 * tirada con backoff exponencial: 1ª inmediato, 2ª +5min, 3ª +30min,
 * 4ª +2h, 5ª +12h. Tras 5 intentos fallidos marca como "failed".
 *
 * Requiere header `Authorization: Bearer <CRON_SECRET>` o ejecutarse
 * desde Vercel Cron (que añade el secret automáticamente).
 */
export async function GET(req: Request) {
  ...
  // Cargar envíos pendientes (con backoff)
  const { data: pending } = await admin
    .from("invoice_aeat_submissions")
    .select(
      "id, company_id, record_id, attempt_number, created_at",
    )
    .eq("status", "pending")
  ...
  for (const sub of submissions) {
    // Backoff: rechazar si aún no toca el siguiente intento
    const delays = [0, 5, 30, 120, 720]; // minutos
    ...
    try {
      ...
      // Cargar líneas de factura para el desglose IVA
      ...
      if (!cs?.verifactu_cert_encrypted || !cs?.verifactu_cert_password_encrypted) {
        await markFailed(
          ...
          sub.id,
          "CERT_MISSING",
          "La empresa no tiene certificado FNMT subido",
        );
        failed++;
        continue;
      }
      ...
      // Marcar como sending y aumentar intento
      await admin
        .from("invoice_aeat_submissions")
        .update({
          status: "sending",
          ...
          request_xml: envelope.length > 200000 ? envelope.slice(0, 200000) : envelope,
        })
        .eq("id", sub.id);
      ...
      console.error("[verifactu-send] cron error:", e);
      await markFailed(
        admin,
        sub.id,
        "EXCEPTION",
